...
import {
  PLATFORM_ACTIVE,
  PLATFORM_TEXT,
  getVipTrackBlockedReason,
  hasRoomVipForPlatform,
  isVipTrackBlocked,
} from '@/lib/platform'
...
export function SearchDialog({ open, onOpenChange, onAddToQueue, onInsertAfterCurrent }: SearchDialogProps) {
  ...
  // Auto re-search on source/type switch
  ...
  const blockedVipCount = useMemo(
    () =>
      searchType === 'song'
        ? (results as Track[]).filter((track) => isVipTrackBlocked(track, platformStatus, statusLoaded)).length
        : 0,
    [results, platformStatus, statusLoaded, searchType],
  )
  ...
  const handleInsertAfterCurrent = useCallback(
    ...
    [onInsertAfterCurrent, queueKeys, addedIds, getTrackAddDisabledReason],
  )
        ...
        <ResponsiveDialogBody className="flex min-h-0 flex-1 flex-col gap-4 overflow-hidden">
          {selectedAlbum ? (
            ...
            <>
              ...
              {searchType === 'song' && statusLoaded && !sourceHasVip && (
                <p className="text-xs text-amber-600">当前房间暂无此平台 VIP 账号，VIP 歌曲将不可添加</p>
              )}
              {searchType === 'song' && statusLoaded && hasSearched && blockedVipCount > 0 && (
